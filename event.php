...
//Add version, date, author, and purpose of the file
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        //Should we check that POST API consumes application/json
        $args = sanitize($_POST);
        $get = sanitize($_GET);
        if (isset($_POST['attach-post-media-submit'])) {
            ...
            if (!wereRequiredFieldsSubmitted($args, $required)) {
                //Consider rephrasing the error message. Include which argument(s) is missing
        ...
        if (isset($args["request_type"])) {
            ...
            // (Only admins and super admins can add another user)
                ...
                require_once('database/dbMessages.php');
                require_once('include/output.php');
                $event = fetch_course_by_id($eventID);
                $eventName = htmlspecialchars_decode($event['name']);
                $eventDate = date('l, F j, Y', strtotime($event['date']));
                $eventStart = time24hto12h($event['startTime']);
                $eventEnd = time24hto12h($event['endTime']);
...
<head>
    ...
    ?>
    <title>Empowerhouse VMS | View Event: <?php echo $event_info['name'] ?></title>
        ...
        <div id="delete-confirmation-wrapper" class="hidden">
            <div id="delete-confirmation">
                ...
                <form method="post" action="deleteEvent.php">
                    ...
                    <input type="hidden" name="id" value="<?= $event_info['eventId'] ?>">
                </form>
                    ...
                    <?php
                        if ($access_level >= 2) {
                            echo '
                                <tr>
                                    <td colspan="2">
                                        <a href="editEvent.php?id=' . $event_info['eventId'] . '" class="button">Edit Event Details</a>
                                    </td>
                                </tr>
                            ';
                        }
            ...
            if ($remaining_slots > 0) {
                if ($access_level >= 2) {
                    if ($event_in_past) {
                        ...
                        $all_volunteers = NULL;// = get_unassigned_available_volunteers($id);
                        if ($all_volunteers) {
